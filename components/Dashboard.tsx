...
const ChartModal: React.FC<{ content: { titleKey: string; unit: string; internalField?: string; externalField?: string; } | null; onClose: () => void; authenticatedFetch: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>; site: Site; currentData: AirDomeData }> = ({ content, onClose, authenticatedFetch, site, currentData }) => {
    const { t } = useAppContext();
...
export const Dashboard: React.FC = () => {
  ...
  const { data, isLoading: isLoadingData, lastUpdated } = useAirDomeData(selectedSite, authenticatedFetch);
  
  const [activeAlerts, setActiveAlerts] = useState<Alert[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [analysis, setAnalysis] = useState('');
  const [isLoadingAnalysis, setIsLoadingAnalysis] = useState(false);
  const [chartModalContent, setChartModalContent] = useState<{ titleKey: string; unit: string; internalField?: string; externalField?: string; } | null>(null);
  ...
  const handleMetricClick = (titleKey: string, field: string, unit: string) => {
    setChartModalContent({ titleKey, unit, internalField: field });
  };

  const handlePairedMetricClick = (titleKey: string, internalField: string, externalField: string, unit: string) => {
