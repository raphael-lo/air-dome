...
export const Alerts: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-brand-dark-light rounded-lg shadow-lg p-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-brand-text mb-4">{t('active_alerts')}</h2>
          ...
          <tbody>
            {alerts.map(alert => (
              <tr key={alert.id} className="border-b border-gray-200 dark:border-brand-dark-lightest hover:bg-gray-50 dark:hover:bg-brand-dark transition-colors">
                ...
                <td className="p-3 text-gray-800 dark:text-brand-text">{t(alert.parameter)}</td>
                <td className="p-3 text-gray-800 dark:text-brand-text">{t(alert.message)}</td>
